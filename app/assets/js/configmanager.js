...
exports.setInstanceFileHash = function(serverid, file, hash) {
    if(!config.instanceFileHashes) {
        config.instanceFileHashes = {}
    }
    if(!config.instanceFileHashes[serverid]) {
        config.instanceFileHashes[serverid] = {}
    }
    config.instanceFileHashes[serverid][file] = hash
}

exports.getInstanceFileHash = function(serverid, file) {
    if(config.instanceFileHashes && config.instanceFileHashes[serverid] && config.instanceFileHashes[serverid][file]) {
        return config.instanceFileHashes[serverid][file]
    } else {
        return null
    }
}
...
exports.getContentVersion = function(serverid, content_type) { 
    const cfgs = config.contentVersions || {}
    if(cfgs[serverid] && cfgs[serverid][content_type]) {
        return cfgs[serverid][content_type]
    } else {
        return null
    }
}

exports.setContentVersion = function(serverid, content_type, version) {
    const currentVersions = config.contentVersions || {};

    if (!currentVersions[serverid]) {
        currentVersions[serverid] = {};
    }
    currentVersions[serverid][content_type] = version;

    config.contentVersions = currentVersions;
}
